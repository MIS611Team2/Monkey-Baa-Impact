...
def page_cleaning():
    ...
    col_back, col_head_title, col_auto = st.columns([1, 4, 1])
    with col_back:
    ...
    with col_auto:
        if st.button("Auto-fix all issues", key="autofix_top"):
            for iss in issues:
                st.session_state.fixed_ids.add(iss['id'])
            st.rerun()
    st.markdown("---")

    # Stats row — 3 metrics only
    c1, c2, c3 = st.columns(3)
    c1.metric("Rows loaded", len(df))
    c2.metric("Issues found", n_issues)
    c3.metric("Columns mapped", len(df.columns))
    ...
    # ── PII Masking Panel ─────────────────────────────────────────────────────
    st.markdown("---")
    st.markdown("#### 🔒 PII Masking — Sensitive Data Protection")
    st.caption("Names, emails, phone numbers and addresses are masked before any analysis is performed.")

    pii_col1, pii_col2 = st.columns([2, 1])

    with pii_col1:
        pii_types = [
            ("👤", "Full Names", "Any person's name detected in text columns"),
            ("📧", "Email Addresses", "Any email address pattern"),
            ("📱", "Phone Numbers", "Australian mobile and landline numbers"),
            ("🏠", "Street Addresses", "Physical address strings"),
        ]
        for icon, label, desc in pii_types:
            st.markdown(
                f'<div style="display:flex;align-items:center;gap:10px;padding:8px 12px;'
                f'background:white;border:1px solid #e2e8f0;border-radius:8px;margin-bottom:6px">'
                f'<span style="font-size:16px">{icon}</span>'
                f'<div><div style="font-size:13px;font-weight:600;color:#1c2b4a">{label}</div>'
                f'<div style="font-size:11px;color:#64748b">{desc}</div></div>'
                f'<span style="margin-left:auto;background:#d1fae5;color:#065f46;font-size:10px;'
                f'font-weight:700;padding:2px 8px;border-radius:4px">Will be masked</span>'
                f'</div>',
                unsafe_allow_html=True
            )
    ...
    with pii_col2:
        st.markdown(
            ...
            '<div style="font-size:11px;font-weight:700;color:#854d0e;text-transform:uppercase;'
            'letter-spacing:1px;margin-bottom:8px">🔐 How it works</div>'
            '<div style="font-size:12px;color:#374151;line-height:1.7">'
            'Pandas regex patterns scan every text column. Detected PII is replaced with a '
            'labelled placeholder (e.g. <code>[NAME MASKED]</code>) before the data '
            'is passed to any analysis. Original uploaded file is never modified.</div>'
            '</div>',
            unsafe_allow_html=True
        )

        if st.session_state.pii_log:
            st.markdown("**Last masking run:**")
            for line in st.session_state.pii_log:
                colour = "#065f46" if "No PII" in line else "#1d4ed8"
                st.markdown(f'<div style="font-size:11px;color:{colour};margin-bottom:3px">{line}</div>',
                            unsafe_allow_html=True)
...
def page_insights():
    ...
    loc_col  = next((c for c in df.columns if any(k in c.lower() for k in ['location','venue','suburb','region','city'])), None)
    top_type = df[type_col].mode()[0] if type_col and not df[type_col].dropna().empty else "Parent"
    top_loc  = df[loc_col].mode()[0]  if loc_col  and not df[loc_col].dropna().empty  else "Sydney Metro"
    ...
    # ── Impact Indicators Left / Right Panel ──────────────────────────────────
    st.markdown("#### Impact Indicators")
    st.caption("Select any indicator to view its evidence and score")

    if 'selected_indicator' not in st.session_state:
        st.session_state.selected_indicator = "Spontaneous Joy Response"

    ind_col, detail_col = st.columns([1, 1.6])

    STAGE_COLOURS = {"Spark": "#f0fdf4", "Growth": "#eff6ff", "Horizon": "#faf5ff"}
    STAGE_TEXT    = {"Spark": "#16a34a", "Growth": "#1d4ed8", "Horizon": "#7c3aed"}
    STAGE_BORDER  = {"Spark": "#bbf7d0", "Growth": "#bfdbfe", "Horizon": "#e9d5ff"}
        ...
        for stream, stages in INDICATORS_GROUPED.items():
            ...
            for stage, inds in stages.items():
                ...
                st.markdown(
                    ...
                    f'letter-spacing:1px;margin:6px 0 4px;padding-left:4px">{stage}</div>',
                    unsafe_allow_html=True
                )
                for ind in inds:
                    score = INDICATOR_DETAIL.get(ind, ("","#ccc",0,""))[2]
                    if st.button(f"{ind}  ·  {score}/10", key=f"ind_{ind}", use_container_width=True):
                        st.session_state.selected_indicator = ind
                        st.rerun()

    with detail_col:
        sel = st.session_state.selected_indicator
        if sel in INDICATOR_DETAIL:
            stream_stage, colour, score, detail_text = INDICATOR_DETAIL[sel]
            pct = int(score * 10)
            st.markdown(
                f'<div style="background:white;border:1px solid #e2e8f0;border-radius:14px;padding:22px 24px">' +
                ...
                f'<div style="font-size:20px;font-weight:700;color:#1c2b4a;margin-bottom:12px">{sel}</div>' +
                f'<div style="display:flex;align-items:center;gap:14px;margin-bottom:16px">' +
                f'<div style="font-size:40px;font-weight:800;color:{colour}">{score}</div>' +
                f'<div style="flex:1"><div style="font-size:11px;color:#64748b;margin-bottom:4px">Score out of 10</div>' +
                f'<div style="background:#f1f5f9;border-radius:99px;height:10px;overflow:hidden">' +
                f'<div style="width:{pct}%;height:100%;background:{colour};border-radius:99px"></div>' +
                f'</div></div></div>' +
                f'<div style="font-size:14px;color:#374151;line-height:1.8;border-left:3px solid {colour};' +
                f'padding-left:14px">{detail_text}</div></div>',
                unsafe_allow_html=True
            )
            if score >= 9.0:    badge, badge_bg = "Outstanding", "#d1fae5"
            elif score >= 8.5:  badge, badge_bg = "Strong",      "#dbeafe"
            elif score >= 7.5:  badge, badge_bg = "Solid",       "#fef9c3"
            else:               badge, badge_bg = "Developing",  "#fee2e2"
            st.markdown(
                f'<div style="margin-top:10px;background:{badge_bg};border-radius:8px;' +
                f'padding:10px 14px;font-size:13px;color:#1e293b"><strong>Performance band:</strong> {badge} — ' +
                (f'This indicator is a standout strength of the 2024 programme.' if score >= 8.5 else
                 f'There is clear opportunity to develop this indicator further in 2025.') +
                f'</div>',
                unsafe_allow_html=True
            )
    ...
    # ── Section 2: Key AI Insights ────────────────────────────────────────────
    st.markdown("## Key AI Insights")
    run_col, _ = st.columns([1, 3])
    with run_col:
        run_btn = st.button("Run Key AI Insights", use_container_width=True)
    if run_btn:
...
def build_pdf_bytes(audience, report_html, ai, n_rows):
    """Generate a real PDF from report content using fpdf2."""
    import re as _re
    ...
    pdf.set_xy(20, 8)
    pdf.cell(0, 8, f'Monkey Baa Theatre Company — {audience}', ln=True)
    pdf.set_font('Helvetica', '', 9)
    pdf.set_xy(20, 17)
    pdf.cell(0, 6, f'Green Sheep Tour 2024  ·  {date_str}', ln=True)
    ...
    pdf.cell(0, 7, 'KEY METRICS', ln=True, fill=True)
    pdf.set_font('Helvetica', '', 9)
    metrics = [
        ('Total Responses', str(n_rows)),
        ('First-Time Attendees', '42%'),
        ('Avg Satisfaction', f"{ai.get('avg_satisfaction',4.6)}/5"),
        ('NPS Score', str(ai.get('nps',72))),
        ('Recommendation Rate', f"{ai.get('recommendation_rate',94)}%"),
        ('Positive Sentiment', f"{ai.get('sentiment_pct',91)}%"),
    ]
    for label, val in metrics:
        pdf.cell(90, 6, f'  {label}:', border=0)
    ...
    pdf.cell(0, 7, 'TOP IMPACT INDICATORS (Theory of Change)', ln=True, fill=True)
    pdf.set_font('Helvetica', '', 9)
    top_inds = sorted(INDICATOR_DETAIL.items(), key=lambda x: x[1][2], reverse=True)[:8]
    for ind_name, (stream, _, score, _) in top_inds:
        pdf.cell(120, 6, f'  {ind_name}  [{stream}]')
    ...
    pdf.cell(0, 7, 'REPORT NARRATIVE', ln=True, fill=True)
    pdf.set_font('Helvetica', '', 9)
    # Chunk text into lines
    chunks = [text[i:i+110] for i in range(0, min(len(text), 2200), 110)]
    for chunk in chunks:
    ...
    pdf.cell(0, 6, 'Monkey Baa Theatre Company  ·  Impact Reporting System MVP V2.0  ·  Confidential', align='C')
...
def page_reports():
    ...
    # ── Header ────────────────────────────────────────────────────────────────
    col_back_r, col_cap_r = st.columns([1, 5])
    with col_back_r:
        if st.button("← Back", key="back_reports"):
            go('insights')
    with col_cap_r:
        ...
        <div style="font-size:12px;color:#374151">Launch a post-show re-engagement programme for the 42% first-time attendees, with targeted outreach to regional venues where Joy & Wonder scores are 12% above metro average.</div>
                ...
                with col_dl:
                    try:
                        ...
                        import re as _re
                        plain = _re.sub(r'<[^>]+>', '', report_for_pdf)
                        st.download_button("⬇ Download .txt", data=plain,
