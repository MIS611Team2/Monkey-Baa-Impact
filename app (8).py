...
def page_insights():
    ...
    # ── Metrics Row ───────────────────────────────────────────────────────────
    # Smarter column detection — avoid long free-text responses
    def best_categorical_col(df, keywords, max_unique=20):
        """Find the column that best represents a categorical variable."""
        candidates = []
        for c in df.columns:
            if any(k in c.lower() for k in keywords):
                nuniq = df[c].dropna().nunique()
                avg_len = df[c].dropna().astype(str).str.len().mean() if not df[c].dropna().empty else 999
                if nuniq <= max_unique and avg_len < 40:
                    candidates.append((c, nuniq, avg_len))
        if candidates:
            # prefer fewer unique values and shorter text
            candidates.sort(key=lambda x: (x[1], x[2]))
            return candidates[0][0]
        return None

    type_col = best_categorical_col(df, ['respondent type','audience type','who are you','role','age group','completed by'], 15)
    loc_col  = best_categorical_col(df, ['suburb','postcode','region','city','area','district'], 30)

    # fallbacks with length guard
    if not type_col:
        for c in df.columns:
            if any(k in c.lower() for k in ['type','respondent','audience']):
                mode_val = df[c].mode()[0] if not df[c].dropna().empty else ""
                if len(str(mode_val)) < 25:
                    type_col = c
                    break

    if not loc_col:
        for c in df.columns:
            if any(k in c.lower() for k in ['location','venue','suburb','region','city']):
                mode_val = df[c].mode()[0] if not df[c].dropna().empty else ""
                if len(str(mode_val)) < 30:
                    loc_col = c
                    break

    raw_type = str(df[type_col].mode()[0]) if type_col and not df[type_col].dropna().empty else "Parent"
    raw_loc  = str(df[loc_col].mode()[0])  if loc_col  and not df[loc_col].dropna().empty  else "Sydney Metro"
    top_type = raw_type[:18] if len(raw_type) > 18 else raw_type
    top_loc  = raw_loc[:18]  if len(raw_loc) > 18  else raw_loc
